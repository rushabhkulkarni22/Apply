...
const state = {user:null, config:{}, locations:[], profile:null, resumes:[], jobs:[], runs:[], usage:{}, route:'overview', filter:'ELIGIBLE', search:''};
...
function chosenLocations(profile){return profile?.preferred_locations?.length?profile.preferred_locations:[profile?.preferred_city||'Pune'];}
function locationPicker(profile){
  const selected=new Set(chosenLocations(profile));
  return `<div class="location-picker span-2"><div class="location-picker-head"><div><strong>Preferred work locations</strong><p>Select cities, entire states, or both. Nearby cities are included using the distance below.</p></div><span>${selected.size} selected</span></div><div class="region-grid">${state.locations.map(region=>`<details class="region"><summary>${esc(region.state)} <small>${region.cities.length} cities</small></summary><label class="check state-check"><input type="checkbox" name="preferred_locations" value="state:${esc(region.state)}" ${selected.has('state:'+region.state)?'checked':''}>Anywhere in ${esc(region.state)}</label><div class="city-checks">${region.cities.map(city=>`<label class="check"><input type="checkbox" name="preferred_locations" value="${esc(city)}" ${selected.has(city)?'checked':''}>${esc(city)}</label>`).join('')}</div></details>`).join('')}</div></div>`;
}
function linkedinSearches(){
  if(!state.profile)return [];
  const keywords=state.profile.target_titles.join(' OR ');
  return chosenLocations(state.profile).slice(0,8).map(place=>{
    const label=place.startsWith('state:')?place.slice(6):place;
    const params=new URLSearchParams({keywords,location:label+', India',f_TPR:'r604800'});
    return {label,url:'https://www.linkedin.com/jobs/search/?'+params.toString()};
  });
}
...
function jobsPage(){
  ...
  linkedinPanel()+`<div class="jobs-grid">${jobs.map(jobCard).join('')}</div>${!jobs.length?empty('No evaluated jobs yet','Open a location search above. When you find a suitable listing, add its description and ApplyWell will verify the title, skills, experience, and location.','<button class="button secondary" data-action="import">Add a job description →</button>'):''}`+
...
function profilePage(){
  const p=state.profile||{full_name:state.user.name,email:state.user.email,experience_years:0,notice_days:30,current_ctc_inr:0,expected_ctc_inr:0,preferred_city:'Pune',preferred_locations:['Pune'],radius_km:100,skills:[],selected_skills:[],target_titles:['Data Engineer'],minimum_skills:3,work_modes:['remote','hybrid']};
...
document.addEventListener('submit',async(event)=>{
  ...
  try{
    ...
    if(form.id==='profile-form'){
      ...
      for(const k of ['experience_years','notice_days','current_ctc_inr','expected_ctc_inr','radius_km','minimum_skills'])body[k]=Number(body[k]);
      body.preferred_locations=values.getAll('preferred_locations');body.preferred_city=(body.preferred_locations.find(v=>!v.startsWith('state:'))||body.preferred_locations[0]?.slice(6)||'India');body.latitude=null;body.longitude=null;
    ...
    if(form.id==='job-form'){
      for(const k of ['minimum_experience','salary_max_inr'])body[k]=body[k]===''?null:Number(body[k]);
...
async function init(){
  try{
    [state.config,{regions:state.locations}]=await Promise.all([api('/config'),api('/locations')]);
